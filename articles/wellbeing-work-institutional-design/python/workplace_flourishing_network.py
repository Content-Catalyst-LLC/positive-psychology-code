"""Network analysis of workplace flourishing.
...
DATA_PATH = BASE_DIR / "data" / "raw" / "workplace_flourishing_network.csv"
...
def draw_network(graph: nx.Graph) -> None:
    ...
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(FIGURE_DIR / "workplace_flourishing_network.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def main() -> None:
    ...
    partial_corr.to_csv(TABLE_DIR / "workplace_flourishing_partial_correlations.csv")
    centrality.to_csv(TABLE_DIR / "workplace_flourishing_network_centrality.csv", index=False)
