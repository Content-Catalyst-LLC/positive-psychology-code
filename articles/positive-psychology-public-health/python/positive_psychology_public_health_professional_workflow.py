"""Professional research scaffold for positive psychology and public health.

This workflow is designed for psychologists, public-health researchers,
social-determinants analysts, and interdisciplinary population well-being teams
who need a transparent, reproducible example of:

- synthetic data ingestion;
- missingness diagnostics;
- indicator-family reliability;
- transparent composite scoring with stress-load penalty;
...
COMPOSITE_WEIGHTS = {
    "life_satisfaction": 0.11,
    "health_index": 0.12,
    "social_trust": 0.10,
    "income_security": 0.10,
    "institutional_quality": 0.11,
    "housing_stability": 0.10,
    "education_access": 0.09,
    "care_access": 0.10,
    "environmental_quality": 0.09,
    "community_resilience": 0.10,
    "stress_load": -0.10,
}

INDICATOR_FAMILIES = {
    "subjective_wellbeing": ["swb1", "swb2", "swb3"],
    "health_functioning": ["health1", "health2", "health3"],
    "social_trust": ["trust1", "trust2", "trust3"],
    "housing_stability": ["housing1", "housing2", "housing3"],
    "care_access": ["care1", "care2", "care3"],
...
def reliability_report(indicator_df: pd.DataFrame) -> pd.DataFrame:
    """Compute reliability diagnostics for synthetic public-health indicator families."""
    rows = []
...
def build_composite_index(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct a transparent weighted public-health flourishing index."""
    result = x_scaled.copy()
    result["public_wellbeing_index"] = 0.0
    ...
    for variable, weight in COMPOSITE_WEIGHTS.items():
        ...
        result["public_wellbeing_index"] += weight * result[variable]
...
def draw_network(graph: nx.Graph, path: Path) -> None:
    ...
    plt.title("Exploratory Network of Positive Psychology and Public Health")
    plt.axis("off")
    plt.tight_layout()
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "positive_psychology_public_health_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "public_health_wellbeing_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "public_health_indicator_bank.csv")
    ...
    indexed.to_csv(TABLE_DIR / "public_health_wellbeing_scaled_index.csv", index=False)
    pca_table.to_csv(TABLE_DIR / "public_health_wellbeing_pca_variance.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "public_health_wellbeing_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "public_health_wellbeing_network_centrality.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "public_health_wellbeing_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "public_health_wellbeing_network.png")

    print("Professional positive psychology and public health workflow complete.")
