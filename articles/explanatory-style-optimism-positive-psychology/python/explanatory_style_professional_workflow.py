"""Professional research scaffold for explanatory style and optimism.
...
and interdisciplinary teams who need a transparent, reproducible example of:

- synthetic explanatory-style data ingestion;
- missingness diagnostics;
- construct-family reliability;
- explanatory-burden, positive-event integration, and context-adjusted agency scoring;
...
INDICATOR_FAMILIES = {
    "negative_stability": ["neg_stability1", "neg_stability2"],
    "negative_globality": ["neg_globality1", "neg_globality2"],
    "negative_personalization": ["neg_personalization1", "neg_personalization2"],
    "positive_stability": ["pos_stability1", "pos_stability2"],
    "positive_globality": ["pos_globality1", "pos_globality2"],
    "positive_effort": ["pos_effort1", "pos_effort2"],
    "agency": ["agency1", "agency2"],
    "support": ["support1", "support2"],
    "persistence": ["persistence1", "persistence2"],
    "hope": ["hope1", "hope2"],
...
def reliability_report(indicator_df: pd.DataFrame) -> pd.DataFrame:
    """Compute reliability diagnostics for synthetic explanatory-style construct families."""
    rows = []
...
def build_explanatory_indices(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct transparent explanatory-style indices for teaching and exploratory analysis."""
    result = x_scaled.copy()

    result["explanatory_burden"] = (
        result["neg_stability"]
        + result["neg_globality"]
        + result["neg_personalization"]
    ) / 3

    result["positive_event_integration"] = (
        result["pos_stability"]
        + result["pos_globality"]
        + result["pos_internal_effort"]
    ) / 3

    result["context_adjusted_agency"] = (
        result["agency_score"]
        + result["support_score"]
        + result["controllability_score"]
        - result["setback_intensity"]
        - result["explanatory_burden"]
    )

    result["resilient_persistence_index"] = (
...
def audit_context(audit_df: pd.DataFrame) -> pd.DataFrame:
    """Create a transparent context audit summary for explanatory-style work."""
    quality_columns = [
    ...
    require_columns(audit_df, quality_columns, "explanatory style context audit dataframe")
...
def pca_summary(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Run PCA for dimensional inspection of attributional-system variables."""
...
def bootstrap_network_centrality(
    ...
    """Bootstrap centrality estimates for exploratory attribution-network stability checks."""
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "explanatory_style_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "explanatory_style_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "explanatory_style_indicator_bank.csv")
    audit_df = pd.read_csv(RAW_DIR / "explanatory_style_context_audit.csv")
    ...
    context_audit.to_csv(TABLE_DIR / "explanatory_style_context_audit_summary.csv", index=False)

    x_scaled = scale_and_impute(network_df, NETWORK_COLUMNS)
    indexed = build_explanatory_indices(x_scaled)
    ...
    indexed.to_csv(TABLE_DIR / "explanatory_style_scaled_indices.csv", index=False)
    pca.to_csv(TABLE_DIR / "explanatory_style_pca_summary.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "explanatory_style_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "explanatory_style_network_centrality.csv", index=False)
    network.edges.to_csv(TABLE_DIR / "explanatory_style_network_edges.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "explanatory_style_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "explanatory_style_network.png")

    print("Professional explanatory style workflow complete.")
