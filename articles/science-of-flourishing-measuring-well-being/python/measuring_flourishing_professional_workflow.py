"""Professional research scaffold for measuring multidimensional flourishing.

This workflow is designed for psychologists, psychometricians, well-being
researchers, and interdisciplinary teams who need a transparent, reproducible
example of:

- synthetic data ingestion;
- missingness diagnostics;
- domain reliability;
- multidimensional flourishing composite scoring;
- measurement-quality auditing;
...
EUD_COLUMNS = ["purpose_life", "personal_growth", "autonomy"]

INDICATOR_FAMILIES = {
    "hedonic": ["hed1", "hed2", "hed3"],
    "eudaimonic": ["eud1", "eud2", "eud3"],
    "relational": ["rel1", "rel2"],
    "accomplishment": ["acc1", "acc2"],
    "health_capacity": ["health1", "health2"],
    "contextual_support": ["context1", "context2"],
...
def reliability_report(indicator_df: pd.DataFrame) -> pd.DataFrame:
    """Compute reliability diagnostics for synthetic item families."""
    rows = []
    ...
    for family_name, columns in INDICATOR_FAMILIES.items():
        ...
        if family_name == "hedonic" and "hed3" in items.columns:
            items = items.copy()
            items["hed3"] = -items["hed3"]
...
def build_indices(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct domain and integrated flourishing indices."""
    result = x_scaled.copy()

    result["hedonic_index"] = (
        result["life_satisfaction"]
        + result["positive_affect"]
        - result["negative_affect"]
    )

    result["eudaimonic_index"] = result[EUD_COLUMNS].mean(axis=1)
    result["relational_index"] = result["positive_relations"]
    result["accomplishment_index"] = result["accomplishment"]
    result["health_capacity"] = result["health_index"]

    result["integrated_flourishing_index"] = (
        0.25 * result["hedonic_index"]
        + 0.25 * result["eudaimonic_index"]
        + 0.15 * result["relational_index"]
        + 0.15 * result["accomplishment_index"]
        + 0.15 * result["health_capacity"]
        + 0.15 * result["contextual_support"]
        - 0.15 * result["stress_load"]
    )

    return result


def audit_measurement_quality(quality_df: pd.DataFrame) -> pd.DataFrame:
    """Create a transparent measurement-quality audit summary."""
    quality_columns = [
    ...
    require_columns(quality_df, quality_columns, "measurement quality dataframe")

    result = quality_df.copy()
    result["computed_quality_mean"] = result[quality_columns].mean(axis=1)
    result["quality_gap"] = result["overall_measurement_quality"] - result["computed_quality_mean"]
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "measuring_flourishing_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "measuring_flourishing_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "flourishing_indicator_bank.csv")
    quality_df = pd.read_csv(RAW_DIR / "measurement_quality_audit.csv")
    ...
    missingness_table(quality_df).to_csv(TABLE_DIR / "measurement_quality_missingness.csv", index=False)

    reliability = reliability_report(indicator_df)
    quality_audit = audit_measurement_quality(quality_df)

    reliability.to_csv(TABLE_DIR / "indicator_family_reliability_report.csv", index=False)
    quality_audit.to_csv(TABLE_DIR / "measurement_quality_audit_summary.csv", index=False)

    x_scaled = scale_and_impute(network_df, NETWORK_COLUMNS)
    indexed = build_indices(x_scaled)
    ...
    indexed.to_csv(TABLE_DIR / "measuring_flourishing_scaled_indices.csv", index=False)
    pca_table.to_csv(TABLE_DIR / "measuring_flourishing_pca_variance.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "measuring_flourishing_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "measuring_flourishing_network_centrality.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "measuring_flourishing_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "measuring_flourishing_network.png")

    print("Professional measuring-flourishing workflow complete.")
