"""Professional research scaffold for positive psychology and sustainability.

This workflow is designed for psychologists, sustainability researchers,
public-health researchers, and interdisciplinary sustainable-flourishing teams
who need a transparent, reproducible example of:

- synthetic data ingestion;
- missingness diagnostics;
- indicator-family reliability;
- direction-corrected sustainable flourishing scoring;
...
COMPOSITE_WEIGHTS = {
    ...
    "social_trust": 0.08,
    "belonging": 0.07,
    "institutional_quality": 0.09,
    "public_service_access": 0.08,
    "ecological_stability": 0.09,
    "health_index": 0.08,
    "mental_health_index": 0.08,
    "adaptive_capacity": 0.08,
    "environmental_exposure": -0.06,
    "insecurity_load": -0.06,
    "inequality_index": -0.06,
}

INDICATOR_FAMILIES = {
    "psychological_functioning": ["psych1", "psych2", "psych3", "psych4"],
    "relational_support": ["rel1", "rel2"],
    "institutional_capacity": ["inst1", "inst2"],
    "health_capacity": ["health1", "health2"],
    "adaptive_capacity": ["adapt1", "adapt2"],
    "cumulative_strain": ["strain1", "strain2"],
}
...
def reliability_report(indicator_df: pd.DataFrame) -> pd.DataFrame:
    """Compute reliability diagnostics for synthetic sustainable-flourishing indicator families."""
    rows = []
...
def build_composite_index(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct a transparent weighted sustainable flourishing index."""
    result = x_scaled.copy()
    result["sustainable_flourishing_index"] = 0.0
    ...
    for variable, weight in COMPOSITE_WEIGHTS.items():
        ...
        result["sustainable_flourishing_index"] += weight * result[variable]
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "positive_psychology_sustainability_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "positive_psychology_sustainability_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "sustainable_flourishing_indicator_bank.csv")
    ...
    indexed.to_csv(TABLE_DIR / "positive_psychology_sustainability_scaled_index.csv", index=False)
    pca_table.to_csv(TABLE_DIR / "positive_psychology_sustainability_pca_variance.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "positive_psychology_sustainability_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "positive_psychology_sustainability_network_centrality.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "positive_psychology_sustainability_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "sustainable_flourishing_network.png")

    print("Professional positive psychology and sustainability workflow complete.")
