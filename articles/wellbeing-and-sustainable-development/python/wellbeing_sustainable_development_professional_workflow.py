"""Professional research scaffold for well-being and sustainable development.

This workflow is designed for psychologists, sustainable-development researchers,
and interdisciplinary well-being teams who need a transparent, reproducible example of:

- synthetic data ingestion;
- missingness diagnostics;
- indicator-family reliability;
- transparent composite scoring with inequality penalty;
...
COMPOSITE_WEIGHTS = {
    ...
    "ecological_stability": 0.12,
    "social_trust": 0.11,
    "resilience_capacity": 0.10,
    "basic_services": 0.10,
    "inequality_index": -0.08,
}
...
def draw_network(graph: nx.Graph, path: Path) -> None:
    ...
    plt.title("Exploratory Network of Well-Being and Sustainable Development")
    plt.axis("off")
    plt.tight_layout()
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "wellbeing_sustainable_development_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "sustainable_development_flourishing_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "sustainable_wellbeing_indicator_bank.csv")
    ...
    indexed.to_csv(TABLE_DIR / "sustainable_flourishing_scaled_index.csv", index=False)
    pca_table.to_csv(TABLE_DIR / "sustainable_development_pca_variance.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "sustainable_development_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "sustainable_development_network_centrality.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "sustainable_development_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "wellbeing_sustainable_development_network.png")

    print("Professional well-being and sustainable development workflow complete.")
