"""Professional research scaffold for Broaden-and-Build Theory.

This workflow is designed for positive psychology researchers, psychologists,
emotion researchers, resilience researchers, educational researchers, organizational
researchers, well-being scientists, and interdisciplinary teams who need a transparent,
reproducible example of:

- synthetic broaden-and-build data ingestion;
- missingness diagnostics;
- mechanism-family reliability;
- broadening, resource, recovery, and net-adaptation composite scoring;
- practice-quality auditing;
...
public-benefits, school disciplinary, employee-evaluation, or individual
...
INDICATOR_FAMILIES = {
    "positive_emotion": ["positive1", "positive2"],
    "negative_emotion": ["negative1", "negative2"],
    "cognitive_flexibility": ["flexibility1", "flexibility2"],
    "exploratory_behavior": ["exploration1", "exploration2"],
    "affiliative_behavior": ["affiliation1", "affiliation2"],
    "social_support": ["support1", "support2"],
    "resilience": ["resilience1", "resilience2"],
    "stress_arousal": ["arousal1", "arousal2"],
    "contextual_safety": ["safety1", "safety2"],
    "resource_stock": ["resource1", "resource2"],
}
...
def reliability_report(indicator_df: pd.DataFrame) -> pd.DataFrame:
    """Compute reliability diagnostics for synthetic broaden-and-build item families."""
    rows = []
    ...
    for family_name, columns in INDICATOR_FAMILIES.items():
        ...
        if family_name in {"negative_emotion", "stress_arousal"}:
            items = -items
...
def build_broaden_build_indices(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct broadening, resource, recovery, and net-adaptation indices."""
    result = x_scaled.copy()

    result["broadening_index"] = (
        result["positive_emotion"]
        + result["cognitive_flexibility"]
        + result["exploratory_behavior"]
        + result["affiliative_behavior"]
        + result["contextual_safety"]
        - result["negative_emotion"]
    ) / 6

    result["resource_index"] = result[RESOURCE_COLUMNS].mean(axis=1)

    result["recovery_capacity_index"] = (
        result["positive_emotion"]
        + result["social_support"]
        + result["contextual_safety"]
        + result["resilience_score"]
        - result["stress_arousal"]
        - result["negative_emotion"]
    ) / 6

    result["net_adaptation_index"] = (
        result["positive_emotion"]
        + result["cognitive_flexibility"]
        + result["exploratory_behavior"]
        + result["affiliative_behavior"]
...
def audit_practice_quality(practice_df: pd.DataFrame) -> pd.DataFrame:
    """Create a transparent broaden-and-build practice-quality audit summary."""
    quality_columns = [
        "emotion_fit",
        "mechanism_clarity",
        "acceptability",
    ...
    result["practice_quality_gap"] = (
        result["overall_practice_quality"] - result["computed_quality_mean"]
    )
...
def pca_summary(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Run PCA for dimensional inspection of broaden-and-build mechanisms."""
    pca_columns = NETWORK_COLUMNS
    n_components = min(5, len(pca_columns), len(x_scaled) - 1)
...
def bootstrap_network_centrality(
    ...
    """Bootstrap centrality estimates for exploratory broaden-and-build mechanism stability checks."""
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "broaden_build_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "broaden_build_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "broaden_build_indicator_bank.csv")
    practice_quality_df = pd.read_csv(RAW_DIR / "broaden_build_practice_quality_audit.csv")
    ...
    practice_audit = audit_practice_quality(practice_quality_df)

    reliability.to_csv(TABLE_DIR / "indicator_family_reliability_report.csv", index=False)
    practice_audit.to_csv(TABLE_DIR / "broaden_build_practice_quality_audit_summary.csv", index=False)

    x_scaled = scale_and_impute(network_df, NETWORK_COLUMNS)
    indexed = build_broaden_build_indices(x_scaled)
    ...
    indexed.to_csv(TABLE_DIR / "broaden_build_scaled_indices.csv", index=False)
    pca.to_csv(TABLE_DIR / "broaden_build_pca_summary.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "broaden_build_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "broaden_build_network_centrality.csv", index=False)
    network.edges.to_csv(TABLE_DIR / "broaden_build_network_edges.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "broaden_build_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "broaden_build_network.png")

    print("Professional Broaden-and-Build workflow complete.")
