"""Professional research scaffold for post-traumatic growth.

This workflow is designed for psychologists, trauma researchers, positive psychology
researchers, counseling researchers, clinical researchers, resilience researchers,
meaning-making researchers, and interdisciplinary teams who need a transparent,
reproducible example of:

- synthetic PTG data ingestion;
- missingness diagnostics;
- construct-family reliability;
- PTG-domain, integration, reflection-balance, growth-distress, and growth-alignment scoring;
- trauma-context support auditing;
...
therapeutic, crisis-support, crisis-triage, workplace-screening, employment-selection,
student-ranking, employee-evaluation, benefits-eligibility, legal, insurance,
...
INDICATOR_FAMILIES = {
    "appreciation": ["appreciation1", "appreciation2"],
    "relationships": ["relationships1", "relationships2"],
    "personal_strength": ["strength1", "strength2"],
    "new_possibilities": ["new_possibilities1", "new_possibilities2"],
    "existential_change": ["existential1", "existential2"],
...
def reliability_report(indicator_df: pd.DataFrame) -> pd.DataFrame:
    """Compute reliability diagnostics for synthetic PTG construct families."""
    rows = []
...
def build_ptg_indices(panel_df: pd.DataFrame) -> pd.DataFrame:
    """Build interpretable PTG indices from raw-scale panel data."""
    require_columns(panel_df, PTG_DOMAIN_COLUMNS + NETWORK_COLUMNS, "PTG panel dataframe")
    result = panel_df.copy()

    result["ptg_domain_mean"] = result[PTG_DOMAIN_COLUMNS].mean(axis=1)
    result["integration_index"] = result[
        ["meaning_making", "restored_agency", "narrative_integration", "social_support", "context_support"]
    ].mean(axis=1)
    result["reflective_processing_balance"] = (
        result["deliberate_rumination"] - result["intrusive_rumination"]
    )
    result["growth_distress_balance"] = (
        result["ptg_score"]
        + result["wellbeing_score"]
        + result["integration_index"]
        - result["distress_score"]
        - result["ongoing_stress"]
    )
    result["growth_alignment"] = (
        result["perceived_growth"]
        + result["corroborated_growth"]
        - (result["perceived_growth"] - result["corroborated_growth"]).abs()
    )
...
def audit_context_support(audit_df: pd.DataFrame) -> pd.DataFrame:
    """Create a transparent trauma-context support audit summary."""
    quality_columns = [
    ...
    require_columns(audit_df, quality_columns, "PTG context support audit dataframe")
...
def pca_summary(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Run PCA for dimensional inspection of PTG recovery-process variables."""
    pca_columns = [
        "assumptive_disruption",
        "intrusive_rumination",
        "deliberate_rumination",
...
def bootstrap_network_centrality(
    ...
    """Bootstrap centrality estimates for exploratory PTG network stability checks."""
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "post_traumatic_growth_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "ptg_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "ptg_indicator_bank.csv")
    audit_df = pd.read_csv(RAW_DIR / "ptg_context_support_audit.csv")
    ...
    scored_panel = build_ptg_indices(panel_df)
    ...
    scored_panel.to_csv(TABLE_DIR / "ptg_scored_panel.csv", index=False)
    ...
    pca.to_csv(TABLE_DIR / "ptg_pca_summary.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "ptg_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "ptg_network_centrality.csv", index=False)
    network.edges.to_csv(TABLE_DIR / "ptg_network_edges.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "ptg_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "ptg_network.png")

    print("Professional post-traumatic growth workflow complete.")
