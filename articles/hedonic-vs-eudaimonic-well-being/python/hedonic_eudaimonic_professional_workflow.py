"""Professional research scaffold for hedonic and eudaimonic well-being.

This workflow is designed for psychologists, well-being researchers, and
interdisciplinary teams who need a transparent, reproducible example of:

- synthetic data ingestion;
- missingness diagnostics;
- domain reliability;
- hedonic, eudaimonic, and integrated composite scoring;
...
INDICATOR_FAMILIES = {
    ...
    "eudaimonic": ["eud1", "eud2", "eud3", "eud4", "eud5", "eud6"],
...
def build_indices(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct hedonic, eudaimonic, and integrated flourishing indices."""
    ...
    result["integrated_flourishing_index"] = (
        0.40 * result["hedonic_index"]
        + 0.45 * result["eudaimonic_index"]
        + 0.20 * result["contextual_support"]
        - 0.20 * result["stress_load"]
    )
...
def main() -> None:
    panel_df = pd.read_csv(RAW_DIR / "hedonic_eudaimonic_panel.csv")
    network_df = pd.read_csv(RAW_DIR / "hedonic_eudaimonic_network.csv")
    indicator_df = pd.read_csv(RAW_DIR / "hedonic_eudaimonic_indicator_bank.csv")
    ...
    indexed.to_csv(TABLE_DIR / "hedonic_eudaimonic_scaled_indices.csv", index=False)
    pca_table.to_csv(TABLE_DIR / "hedonic_eudaimonic_pca_variance.csv", index=False)
    network.partial_correlations.to_csv(TABLE_DIR / "hedonic_eudaimonic_partial_correlations.csv")
    network.centrality.to_csv(TABLE_DIR / "hedonic_eudaimonic_network_centrality.csv", index=False)
    bootstrap_summary.to_csv(TABLE_DIR / "hedonic_eudaimonic_bootstrap_centrality.csv", index=False)

    draw_network(network.graph, FIGURE_DIR / "hedonic_eudaimonic_network.png")

    print("Professional hedonic/eudaimonic workflow complete.")
