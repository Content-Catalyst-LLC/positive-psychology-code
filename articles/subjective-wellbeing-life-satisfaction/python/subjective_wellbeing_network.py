"""Composite and network analysis of subjective well-being.
...
DATA_PATH = BASE_DIR / "data" / "raw" / "subjective_wellbeing_network.csv"
...
def build_swb_index(x_scaled: pd.DataFrame) -> pd.DataFrame:
    """Construct a transparent subjective well-being index."""
    result = x_scaled.copy()
    result["swb_index"] = (
        0.40 * result["life_satisfaction"]
        + 0.35 * result["positive_affect"]
        - 0.35 * result["negative_affect"]
    )
    result["contextual_swb_index"] = (
        0.30 * result["life_satisfaction"]
        + 0.25 * result["positive_affect"]
        - 0.25 * result["negative_affect"]
        + 0.10 * result["social_support"]
        + 0.08 * result["income_security"]
        + 0.10 * result["meaning_alignment"]
        - 0.08 * result["stress_load"]
    )
    return result
...
def draw_network(graph: nx.Graph) -> None:
    ...
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(FIGURE_DIR / "subjective_wellbeing_network.png", dpi=300, bbox_inches="tight")
    plt.close()


def main() -> None:
    x_scaled = load_and_scale(DATA_PATH)
    indexed = build_swb_index(x_scaled)
    partial_corr, centrality = build_partial_correlation_network(x_scaled)

    indexed.to_csv(TABLE_DIR / "subjective_wellbeing_scaled_index.csv", index=False)
    partial_corr.to_csv(TABLE_DIR / "subjective_wellbeing_partial_correlations.csv")
    centrality.to_csv(TABLE_DIR / "subjective_wellbeing_network_centrality.csv", index=False)
